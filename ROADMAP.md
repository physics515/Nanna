...
- [ ] **The Tauri GUI does not build on this Linux host, and it is not our code.** `cargo check -p
      ...
      **Root cause, traced:** `tauri_build::copy_binaries` does
      ...
      this cargo.
      - [ ] Fix options, cheapest first: pin cargo/nightly to one still emitting the classic layout;
      ...
      - [ ] `gui/src-tauri/gen/schemas/linux-schema.json` is generated by a Linux build and is not
