//! Tool authoring module - create, edit, test, and manage user-defined tools
//!
//! Allows both GUI-based tool creation and runtime tool creation by Nanna herself.
...
// ============================================================================
// CreateToolTool - Allows Nanna to create tools at runtime
// ============================================================================

/// Tool that allows the agent to create new tools at runtime
pub struct CreateToolTool {
    manager: Arc<UserToolManager>,
    registry: Arc<ToolRegistry>,
}

impl CreateToolTool {
    pub fn new(manager: Arc<UserToolManager>, registry: Arc<ToolRegistry>) -> Self {
...
impl Tool for CreateToolTool {
    fn definition(&self) -> ToolDefinition {
        ToolDefinition {
            ...
            parameters: vec![
                ToolParameter {
                    name: "name".to_string(),
                    description: "Unique name for the tool (lowercase, underscores allowed)".to_string(),
                ...
                ToolParameter {
                    name: "parameters_schema".to_string(),
                    description: "Optional JSON Schema for input parameters".to_string(),
    ...
    async fn execute(&self, params: HashMap<String, Value>) -> Result<ToolResult, ToolError> {
        ...
        let code = params.get("code")
            .and_then(|v| v.as_str())
            .ok_or_else(|| ToolError::InvalidParams("code is required".to_string()))?
            .to_string();
...
// ============================================================================
// ListToolsTool - Allows Nanna to see available user tools
// ============================================================================
...
impl ListUserToolsTool {
    pub fn new(manager: Arc<UserToolManager>) -> Self {
        Self { manager }
    }
}
...
impl Tool for ListUserToolsTool {
    ...
    async fn execute(&self, _params: HashMap<String, Value>) -> Result<ToolResult, ToolError> {
        ...
        if tools.is_empty() {
            return Ok(ToolResult::success("No user-created tools found."));
        }

        let mut output = format!("Found {} user-created tools:\n\n", tools.len());
        for tool in tools {
            output.push_str(&format!(
                "• **{}** {}\n  {}\n\n",
                tool.name,
                if tool.enabled { "✓" } else { "(disabled)" },
                tool.description
            ));
        }
