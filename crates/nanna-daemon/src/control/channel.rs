...
impl ControlPlane {
    ...
    pub(super) async fn handle_channel(&self, _client_id: &str, action: ChannelAction) -> Value {
        match action {
            ChannelAction::List => self.channel_list().await,
            ChannelAction::Status { id } => self.channel_status(id).await,
    ...
    /// `ChannelAction::List`: which channels have credentials configured.
    async fn channel_list(&self) -> Value {
    ...
    /// `ChannelAction::Status`: one channel's live status, or all of them with
    /// a summary.
    async fn channel_status(&self, id: Option<String>) -> Value {
        ...
        if let Some(channel_id) = id {
            status_manager.get(&channel_id).await.map_or_else(
                || {
                    json!({
                        "error": "not_found",
                        "channel_id": channel_id,
