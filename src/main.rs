...
async fn handle_credentials_command(action: CredentialsAction) -> anyhow::Result<()> {
    ...
    match action {
        ...
        CredentialsAction::Import => {
            ...
            match manager.load() {
                Ok(loaded) => {
                    ...
                    // Update Nanna config to use OAuth
        ...
        CredentialsAction::Setup => {
            ...
            // Import the credentials that were just created
            match manager.load() {
                Ok(loaded) => {
                    // Update Nanna config
                    let mut config = Config::load().unwrap_or_default();
...
async fn handle_daemon_command(action: DaemonAction, _config: &Config) -> anyhow::Result<()> {
...
fn stop_daemon_process(pid_file: &std::path::Path) -> anyhow::Result<()> {
    ...
    #[cfg(not(windows))]
    {
        ...
        match signal::kill(Pid::from_raw(pid as i32), Signal::SIGTERM) {
            Ok(()) => println!("✅ Daemon stopped (PID {pid})"),
            Err(e) => {
